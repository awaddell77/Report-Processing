...
#tested
class Mtnplbg_spiff(Spiff_report):
	...
	def transform_eclipse(self, t_dict, key):
		...
		if len(temp) == 1: t_dict["CatMatch"] = temp[0].split('/')[0]
		elif temp[0:1] == ['MOUNTAIN', 'PLUMBING']: t_dict['CatMatch'] = temp[2].split('/')[0]
		else: t_dict["CatMatch"] = temp[1].split('/')[0]
